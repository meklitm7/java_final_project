...
public class ReportGenerator {
    ...
    public void generateCompanyActivityReport(String filePath) {
        String sql = "SELECT id, name, location, account_number, telebirr_phone, admin_telegram FROM Companies";
        writeReport(sql, filePath);
    }

    // NEW: Volunteer report method
    public void generateVolunteerReport(String filePath) {
        String sql = "SELECT id, name, contact, status FROM Volunteers";
        writeReport(sql, filePath);
    }
    ...
    private void writeReport(String sql, String filePath) {
        try (Statement stmt = conn.createStatement();
            ...
            throw new RuntimeException(e); // Re-throw to show in UI
        }
    }
}